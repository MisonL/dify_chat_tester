...
- **报错信息优化**：增强 `_friendly_error_message`，识别 "inappropriate content" 等内容安全错误，统一返回中文提示："请求内容可能包含敏感信息，被模型拒绝处理。"
- **统计结果显示**：批量处理完成后的“总用时长”字段支持自动转换为分钟/小时单位（例如 "10 小时 45 分 29 秒"），不再只显示总秒数。
- **键盘交互体验**：优化 Q 键中断操作，按下后立即刷新 UI 显示"正在停止"状态，无需等待轮询周期。
- **代码风格**：全项目执行 `ruff format`，统一代码格式规范。

### 修复

- **串行模式 UI**：修复了串行模式下按 Ctrl+C 中断时警告图标重复显示的问题。
- **错误处理增强**：`_friendly_error_message` 增强了对 Dify 数据库连接池错误的识别精确度，避免将普通网络超时误判为连接池错误。

### 测试

- **测试覆盖**：新增串行模式单元测试 (`TestRunSequentialBatch`) 和 UI 逻辑测试 (`tests/test_cli_ui.py`)，整体测试通过率 100% (39/39)。
