...
- **钱小烎插件 (v0.2.0)**：
  - **自动配置**：默认使用 `auto` 模型和 `user` 角色，无需用户手动选择。
  - **流式优化**：修复了流式响应结束后内容可能消失的问题，现在会持久化显示。
  - **多轮对话**：修复了上下文丢失问题，完美支持连续对话。
  - **UI 增强**：支持显示详细的工具调用状态（运行中/成功/失败）和推理过程（Reasoning）。
  - **稳定性**：修复了空响应可能导致的程序崩溃 Bug。
- **示例插件 (Demo Plugin)**：
