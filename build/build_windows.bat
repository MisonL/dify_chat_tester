...
echo ==========================================
echo Dify Chat Tester Windows 打包脚本
echo ==========================================
...
if not exist "%PROJECT_DIR%\release_windows\dify_chat_tester.exe" goto :FAILURE

:SUCCESS
echo.
echo 构建成功！
echo 可执行文件位置: %PROJECT_DIR%\release_windows\dify_chat_tester.exe
...
echo.
echo 使用说明:
echo 1. 解压 dify_chat_tester_windows_v*.zip
echo 2. 复制 .env.config.example 为 .env.config
echo 3. 编辑 .env.config 配置 API 信息
echo 4. 双击 dify_chat_tester.exe 启动程序
echo.
echo 打包完成！
