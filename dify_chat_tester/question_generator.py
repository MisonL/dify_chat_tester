...
请生成问题列表（必须是JSON数组格式，例如:["问题1","问题2","问题3"]）："""
        ...
        try:
            ...
            if questions:
                print_success(f"文档 {document_name} - 分块 {idx}/{total_chunks} 生成 {len(questions)} 个问题")
                all_questions.extend(questions)
            else:
                print_warning(f"文档 {document_name} - 分块 {idx}/{total_chunks} 未能解析出问题")
        except Exception as e:
    ...
    if dedup_questions:
        print_success(f"文档 {document_name} 共生成 {len(dedup_questions)} 个去重后的问题")
    else:
...
def export_questions_to_excel(questions_data: List[Tuple[str, str]], output_path: str) -> bool:
    """
    将问题数据导出到Excel
