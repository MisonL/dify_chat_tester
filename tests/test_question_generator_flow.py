...
class TestQuestionGeneratorExtended:
    ...
    @patch("dify_chat_tester.question_generator.read_markdown_files")
    @patch("dify_chat_tester.question_generator.generate_questions_for_document")
    @patch("dify_chat_tester.question_generator.export_questions_to_excel")
    def test_run_question_generation_flow(
        self,
        mock_export,
        mock_generate,
        mock_read
    ...
    @patch("dify_chat_tester.question_generator.read_markdown_files")
    @patch("dify_chat_tester.question_generator.print_error")
    def test_run_question_generation_no_files(self, mock_error, mock_read):
        """测试没有 Markdown 文件"""
        mock_read.return_value = ([], {})
        
        run_question_generation(
            MagicMock(), "user", "Dify", "model", "dummy_path"
        )
